...
def link_points_to_ipums_and_world(country_code,
    ...
    point_df = pd.read_csv(point_df_path, encoding="latin1", na_values=[-999, "-999"])

    if country_code!="BFA":
        point_df["geometry"] = [Point(xy) for xy in zip(point_df[lon_column], point_df[lat_column])]
        point_df = gpd.GeoDataFrame(point_df, crs="epsg:4326").set_geometry('geometry')    
        point_df = point_df.sjoin(world_shapefile, how="left", predicate="within")
        point_df = point_df.rename(columns={"index_right":"index_world_shapefile"})
        point_df = point_df.sjoin(ipums_data, how="left", predicate="within")
        point_df = point_df.rename(columns={"index_right":"index_ipums_terra"})
    ...
    if country_code=="BFA":
        ...
        point_df = point_df.merge(world_shapefile, how="left", left_on="iso_3",right_on="iso_a3")
